...
class tdLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        x : (N,C,T)
        """
        # print("tdLinear forward x.shape self.weight.shape self.bias", x.shape, self.weight.shape, self.bias.shape)
...
class tdConv(nn.Conv3d):
    ...
    def forward(self, x):
        # print("tdConv forward x.shape self.weight.shape, self.bias.shape, self.stride, self.padding, self.dilation, self.groups", x.shape, self.weight.shape, self.bias.shape, self.stride, self.padding, self.dilation, self.groups)
...
class tdBatchNorm(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        if self.training:
            # print("tdBatchNorm forward input.shape", input.shape)
            if input.dim() == 5:  # 5D input (e.g., N, C, D, H, W)
                mean = input.mean([0, 2, 3, 4])
                var = input.var([0, 2, 3, 4], unbiased=False)
            elif input.dim() == 4:  # 4D input (e.g., N, C, H, W)
